 ...
 *
 * GET  /api/golddigger/watchlist — Returns all watched symbols with latest quotes
 * POST /api/golddigger/watchlist — Add a symbol to watchlist (body: { symbol: string })
 * DELETE /api/golddigger/watchlist — Remove a symbol from watchlist (body: { symbol: string })
 */
...
async function fetchQuote(symbol: string): Promise<QuoteData> {
  ...
  try {
    ...
    const data: { chart?: { result?: Array<{ timestamp?: number[]; indicators?: { quote?: Array<{ close?: number[] }> } }> } } = await response.json();
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validate format: 1-10 chars — letters, digits, hyphens, dots, carets (e.g. AAPL, BTC-USD, ^VIX, BRK.B)
    if (!/^[\^]?[A-Z0-9]{1,5}([.\-][A-Z0-9]{1,4})?$/.test(normalizedSymbol)) {
      return NextResponse.json(
        { error: "Invalid symbol format. Examples: AAPL, BTC-USD, ^VIX, BRK.B" },
        { status: 400 }
      );
    }
