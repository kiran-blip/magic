...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.warn("[Gold Digger Memory] Recall failed:", err);
    }
    ...
    try {
      ...
      if (result.agentType === "investment" && responseText.length > 50) {
        try {
          ...
          if (actionMatch && actionMatch[1] && actionMatch[2]) {
            ...
            console.log(`[Gold Digger Memory] Stored investment decision: ${action} ${symbol} @ ${confidence}%`);
          }
        } catch {
      ...
      console.warn("[Gold Digger Memory] Store failed:", err);
    }
    ...
    console.error("[Gold Digger API] Pipeline error:", err);
