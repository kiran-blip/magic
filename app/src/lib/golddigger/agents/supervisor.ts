 ...
 * Ported from jarvis-v4/src/nodes/supervisor_node.py + src/graphs/supervisor.py (now Gold Digger)
 *
...
function governorNode(state: AgentState): AgentState {
  ...
  if (result.blocked) {
    ...
    if (result.sanitizeCredentials) {
      console.warn("[Gold Digger] Credentials detected in query — sanitizing before LLM");
      state.query = sanitizeText(state.query);
    }
    // Redact PII from the query before the LLM sees it too
    if (result.redactPii) {
      console.warn("[Gold Digger] PII detected in query — redacting before LLM");
...
function endNode(state: AgentState): AgentState {
  ...
  if (meta.redactPii) {
    console.warn("[Gold Digger] PII detected — redacting from response");
...
export interface GoldDiggerOptions {
...
/**
 * Run the full Gold Digger pipeline on a user message.
 *
 ...
 */
export async function runGoldDigger(
  query: string,
  history: Array<{ role: "user" | "assistant" | "system"; content: string }> = [],
  options: GoldDiggerOptions = {}
): Promise<AgentState> {
  // Initialize state
