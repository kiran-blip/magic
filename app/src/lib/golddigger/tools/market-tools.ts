 ...
 */
async function fetchYahooFinance(url: string): Promise<Record<string, unknown>> {
...
function extractChartData(
  ...
  try {
    ...
    // Calculate change
    let changePercent = 0;
    if (closes.length > 1 && closes[0] !== null) {
      changePercent =
...
export async function getCryptoData(symbol: string): Promise<CryptoDataResult> {
  try {
    ...
    const chart = data.chart as Record<string, unknown> | undefined;
    const chartResults = (chart?.result as Array<Record<string, unknown>>) ?? [];
    if (!chartResults[0]) {
      return {
    ...
    const quote = quoteArr[0] ?? {};
    const closeData = quote.close as number[] | undefined;
    const volumeData = quote.volume as number[] | undefined;
...
export async function getRelatedAssets(
  ...
  try {
    ...
    try {
      ...
      const summaryResults = (quoteSummary?.result as Array<Record<string, unknown>>) ?? [];
      const assetProfile = summaryResults[0]?.assetProfile as Record<string, unknown> | undefined;

      if (assetProfile?.industry) {
        const industry = assetProfile.industry as string;
